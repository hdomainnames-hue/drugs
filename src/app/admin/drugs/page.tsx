  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">الأدوية</h1>
          <p className="mt-2 text-sm leading-7 text-zinc-600 dark:text-zinc-400">تعديل بيانات الأدوية لتحسين جودة المحتوى.</p>
        </div>
        <Link
          href="/ar/drugs"
      ...
      <form className="flex gap-2" action="/admin/drugs" method="get">
        <input
          name="q"
          defaultValue={q}
          placeholder="بحث بالاسم / الشركة / المادة الفعالة"
      ...
      <div className="overflow-hidden rounded-3xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
        ...
        {items.length ? (
          ...
          <div className="p-6 text-sm text-zinc-600 dark:text-zinc-400">لا توجد نتائج.</div>
        )}
      </div>

      <div className="flex items-center justify-between text-xs text-zinc-600 dark:text-zinc-400">
        <div>
          الصفحة {page} من {totalPages} — {total} عنصر
        </div>
